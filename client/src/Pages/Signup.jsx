...
const Signup = () => {
  ...
  const createNewAccount = async (event) => {
    ...
    // TODO: toast the specific error field
    if (
    ...
    // checking the first name field length
    if (signUpData.firstName.length < 5 || signUpData.firstName.length > 20) {
      toast.error(
        ' First Name character should be more than 5 and less than 20 characters'
      );
      return;
    }
    ...
    if (
      ...
      signUpData.mobileNumber.length > 12
    ) {
      toast.error('Provided mobile number length should be 10 or 12 digit');
      return;
    }
    ...
    const res = await dispatch(createAccount(userData));
      ...
      <section className="text-gray-600 body-font">
        <div className="container flex flex-wrap items-center h-screen px-10 py-6 mx-auto">
          ...
          >
            ...
            <button
              className="px-8 py-2 text-lg text-white bg-yellow-500 border-0 rounded focus:outline-none hover:bg-yellow-600"
              type="submit"
            >
              Create Account
            </button>
